...
public function anular($idingreso){
	...
	$sql="UPDATE ingreso SET estado='Anulado' WHERE idingreso='$idingreso'";
 	...
 	$sql_detalle="UPDATE detalle_ingreso SET estado='0' WHERE idingreso='$idingreso'"; 	
 	ejecutarConsulta($sql_detalle) or $sw=false;
...
public function listarDetalle($idingreso){
	$sql="SELECT di.idingreso,di.idarticulo,a.nombre,di.cantidad,di.precio_compra,di.precio_venta,i.impuesto ,i.total_compra FROM detalle_ingreso di INNER JOIN articulo a ON di.idarticulo=a.idarticulo INNER JOIN ingreso i ON i.idingreso=di.idingreso WHERE di.idingreso='$idingreso'"; 
	return ejecutarConsulta($sql);
}
